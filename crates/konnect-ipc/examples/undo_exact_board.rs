...
fn main() -> Result<()> {
    ...
    if !is_smartmaf {
        bail!(
            "refusing undo: requested file is not SmartMAF.kicad_pcb: '{}'",
            requested.display()
        );
    }
    ...
    if open_boards.len() != 1 {
        bail!(
            "refusing undo: expected exactly one open PCB, found {} ({})",
            open_boards.len(),
            open_boards
                .iter()
    ...
    client.find_open_board(&requested).with_context(|| {
        format!(
            "refusing undo because the sole open PCB is not '{}'",
            requested.display()
        )
    })?;

    client.run_action("common.Interactive.undo")?;
    println!(
        "KiCad accepted one undo action for '{}'.",
        requested.display()
    );
    Ok(())
}
